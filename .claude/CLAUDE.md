...
## Codebase Intelligence for ZaliznjakVideo (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-01 (commit cb4b2c0). Confidence: 100%.
...
### Architecture
**Files:** 3 | **Lines:** 70
ZaliznjakVideo is a yaml codebase of 3 files. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on. ---
*Built from the code's structure. It states what is there, not why it is that
way.

### Code health
Three co-equal signals: defect risk 10.0/10 avg, hotspot health 10.0/10 (stable), worst `.github/dependabot.yml` at 10.0/10 · maintainability 10.0/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
